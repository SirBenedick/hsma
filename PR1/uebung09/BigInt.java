...
//		println("BigInt: "+ number.numberArray[0]);
	...
	void times(BigInt number) {
		BigInt timedFinal = new BigInt("0");

		// Soll jede Stelle im Array durchgehen
		for (int j = 0; j < numberArray.length; j++) {
			if (numberArray[j] != 0) {
				BigInt timed = new BigInt(number.toString());
				...
				BigInt helperObject = new BigInt(stringNumber);
	...
	public static void main(String[] args) {
		...
		BigInt zahl1 = new BigInt("219999"); 
		BigInt zahl2 = new BigInt("999999999999");
		zahl1.ausgabe();
		zahl2.ausgabe();
